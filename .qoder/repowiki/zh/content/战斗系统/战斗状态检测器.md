...
- [skill_controller.py:24-621](file://src/combat/skill_controller.py#L24-L621)
- [BackgroundManager.py:7-155](file://src/utils/BackgroundManager.py#L7-L155)

**章节来源**
- [state_detector.py:1-473](file://src/combat/state_detector.py#L1-L473)
...
**更新** 实现了智能日志节流机制，通过自适应日志间隔减少日志输出频率，同时增强了自身检测的进度报告功能
...
**更新** 智能日志节流机制和增强的自身检测进度报告功能显著减少了日志输出频率，提高了系统的可维护性，同时提供了更好的调试体验。
...
**更新** get_battlefield_state_detailed方法现在确保使用同一帧进行友方和敌方检测，防止竞态条件；detect_self方法实现了智能日志节流机制，通过last_log_time控制日志输出频率

**章节来源**
- [state_detector.py:72-473](file://src/combat/state_detector.py#L72-L473)
- [labels.py:8-51](file://src/combat/labels.py#L8-L51)
...
- 参数说明
  - min_distance/max_distance/buffer：距离范围与缓冲区（默认0/200/15）
- 返回值类型
  - float：距离（像素）
  - str：移动方向
