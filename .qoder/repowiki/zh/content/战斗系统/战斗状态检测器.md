...
**更新** 增强了帧一致性机制，确保友方和敌方检测使用同一帧数据，防止竞态条件
...
## 结论
战斗状态检测器通过YOLO模型与多组件协同，实现了对战场的实时感知与智能决策。并行死亡监控线程、滞回距离判断与后台模式支持共同保障了系统的稳定性与响应速度。结合配置化的技能与移动控制，系统可在不同场景下高效运行。

**更新** 增强的帧一致性机制进一步提升了检测准确性，防止了竞态条件导致的误判，使系统在复杂战场环境下表现更加稳定可靠。
...
**更新** get_battlefield_state_detailed方法现在确保使用同一帧进行友方和敌方检测，防止竞态条件

**章节来源**
- [state_detector.py:72-467](file://src/combat/state_detector.py#L72-L467)
- [labels.py:8-51](file://src/combat/labels.py#L8-L51)
