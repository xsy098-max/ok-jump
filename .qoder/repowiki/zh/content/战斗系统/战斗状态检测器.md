...
## 更新摘要
**变更内容**
- 实现了智能日志节流机制，通过自适应日志间隔减少日志输出频率
- 增强了自身检测的进度报告功能，提供详细的检测过程信息
- 优化了日志输出策略，区分普通详细日志和进度报告日志
- 增强了帧一致性机制，确保友方和敌方检测使用同一帧数据
- 改进了异常处理机制，增加了更详细的错误日志记录
- 优化了死亡监控线程的错误处理和状态管理
...
- 自身位置检测
- 友方与敌方单位检测
- 并行死亡监控线程的工作原理（30ms检测间隔、连续检测机制、防误判策略）
...
**图表来源**
- [state_detector.py:24-496](file://src/combat/state_detector.py#L24-L496)
...
**章节来源**
- [state_detector.py:1-496](file://src/combat/state_detector.py#L1-L496)
...
- 战斗状态检测器（StateDetector）
  ...
  - **智能日志节流机制**：通过自适应日志间隔减少日志输出频率
  - **增强进度报告**：提供详细的检测过程信息
  - **增强功能**：确保友方和敌方检测使用同一帧数据，防止竞态条件
  - **改进的异常处理**：增加了详细的错误日志记录和异常捕获
...
**更新** 实现了智能日志节流机制，通过自适应日志间隔减少日志输出频率，同时增强了自身检测的进度报告功能。改进了异常处理机制，增加了更详细的错误日志记录。
...
- **智能日志节流优化**
  ...
  - 区分普通详细日志和进度报告日志，减少冗余信息
- **改进的异常处理**
  - 死亡监控线程增加了异常捕获和错误日志记录
  - 自身检测增加了详细的异常处理和状态恢复机制
  - 提高了系统的稳定性和可维护性
...
## 结论
战斗状态检测器通过YOLO模型与多组件协同，实现了对战场的实时感知与智能决策。并行死亡监控线程、滞回距离判断、后台模式支持以及智能日志节流机制共同保障了系统的稳定性与响应速度。结合配置化的技能与移动控制，系统可在不同场景下高效运行。

**更新** 智能日志节流机制和增强的自身检测进度报告功能显著减少了日志输出频率，提高了系统的可维护性，同时提供了更好的调试体验。改进的异常处理机制进一步提升了系统的稳定性和可靠性。
...
**更新** get_battlefield_state_detailed方法现在确保使用同一帧进行友方和敌方检测，防止竞态条件；detect_self方法实现了智能日志节流机制，通过last_log_time控制日志输出频率。改进了异常处理机制，增加了详细的错误日志记录。

**章节来源**
- [state_detector.py:72-496](file://src/combat/state_detector.py#L72-L496)
- [labels.py:8-51](file://src/combat/labels.py#L8-L51)
