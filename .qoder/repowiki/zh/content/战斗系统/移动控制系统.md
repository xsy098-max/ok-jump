...
## 更新摘要
**变更内容**
- 移动控制器日志精简：删除了约46行冗余日志，包括屏幕中心位置计算日志、移动执行日志和中断检查日志
- 保留必要调试信息：仅保留关键错误日志和状态信息，提升运行效率
- 优化日志输出：减少日志噪声，专注于重要信息
...
**更新** 本次更新重点介绍了移动控制器日志的精简优化，删除了约46行冗余日志，保留必要的调试信息，提升了系统运行效率。

## 项目结构
移动控制系统位于 src/combat 目录，围绕 AutoCombatTask 任务调度，结合状态检测、技能控制与距离计算，形成"检测-决策-执行"的闭环。关键文件与职责如下：
- movement_controller.py：移动控制器，负责 WASD 键盘与虚拟摇杆的移动控制，支持后台模式与 ADB 模式，新增速度跟踪和中断操作
...
- AutoCombatTask.py：自动战斗任务，编排状态检测、移动控制与技能释放，支持动态移动时间计算
...
subgraph "控制执行层"
MC["MovementController<br/>移动控制<br/>速度跟踪 + 中断操作<br/>精简日志输出"]
...
- 移动控制器（MovementController）
  ...
  - 支持可中断移动与按方向键持续时间控制
  - **新增**：移动速度跟踪机制，支持基于实际移动速度的精准距离调整
  - **新增**：可中断移动功能，允许在移动过程中实时检测停止条件
  - **优化**：日志精简，删除冗余调试信息，保留关键错误和状态日志
...
**更新** 移动控制器现在集成了速度跟踪和中断操作两大核心功能，显著提升了移动控制的智能化水平。同时进行了日志优化，删除了约46行冗余日志，提升了系统运行效率。
...
- MovementController 根据战场态势与距离阈值选择移动方向与持续时间，**新增**：支持基于速度跟踪的动态移动时间计算和可中断移动
- **优化**：移动控制器日志精简，减少调试噪声，提升性能
...
Note over MC : 基于速度跟踪计算<br/>动态移动时间
MC->>MC : move_with_interrupt_check<br/>实时检测停止条件
MC-->>Task : 移动执行完成/中断
```
...
- [skill_controller.py:254-277](file://src/combat/skill_controller.py#L254-277)
...
- 功能要点
  ...
  - **新增**：速度跟踪机制，记录移动前后的坐标和时间，计算实际移动速度
  - **新增**：动态移动时间计算，基于实际速度和目标距离计算精确的移动持续时间
  - **优化**：日志精简，删除冗余的屏幕中心位置计算日志、移动执行日志和中断检查日志
- 关键算法
  - 方向计算：阈值过滤微小偏移，避免无效移动
  - 虚拟摇杆：方向向量归一化后乘以半径，实现全速移动
  - **新增**：速度采样：记录最近 N 次移动的速度，计算平均速度
  - **新增**：动态时间计算：根据剩余距离和平均速度计算移动时间
- 参数与配置
  - move_duration：每次移动按键持续时间，影响移动距离
  - ADB 模式：使用 swipe 循环实现连续移动
  - 分辨率适配：摇杆中心与半径随分辨率缩放
  - **新增**：速度采样窗口：默认10次滚动采样
  - **优化**：日志输出精简，仅保留关键错误和状态信息
...
Keys --> Interrupt{"是否支持中断?"}
Interrupt --> |是| InterruptCheck["move_with_interrupt_check"]
Interrupt --> |否| NormalMove["普通移动"]
InterruptCheck --> CheckLoop["定期检查停止条件"]
CheckLoop --> StopCond{"停止条件满足?"}
StopCond --> |是| Interrupted["中断移动"]
StopCond --> |否| Continue["继续移动"]
Continue --> SpeedTrack["速度跟踪"]
SpeedTrack --> DynamicTime["动态时间计算"]
DynamicTime --> Duration["计算移动持续时间"]
Interrupted --> End(["结束"])
...
- [movement_controller.py:729-800](file://src/combat/movement_controller.py#L729-L800)
...
- 功能要点
  ...
  - **新增**：基于速度跟踪的动态移动时间计算，支持可中断移动
  - **优化**：移动控制器日志精简，减少调试噪声
- 关键流程
  - 死亡状态检测（并行）→ 自身检测 → 战场状态判断 → 距离更新 → 技能/移动控制
  - **新增**：移动前记录位置 → 执行移动 → 采集新位置 → 记录速度采样 → 计算动态时间
  - **优化**：日志输出更加简洁，仅保留关键信息
...
- [AutoCombatTask.py:1200-1399](file://src/task/AutoCombatTask.py#L1200-L1399)
...
- **新增**：速度跟踪优化
  ...
  - 动态时间计算，减少过度移动或移动不足
- **优化**：日志精简优化
  - 删除约46行冗余日志，减少 I/O 开销
  - 保留关键错误日志和状态信息，不影响调试能力
  - 提升系统整体运行效率
...
- **新增**：速度跟踪异常
  - 现象：移动时间计算不准确或移动距离异常
  - 排查：检查速度采样窗口大小、分辨率缩放和移动前后的坐标获取
  - 参考路径：[速度跟踪实现:729-800](file://src/combat/movement_controller.py#L729-L800)
- **新增**：中断移动失效
  - 现象：移动过程中无法响应停止条件
  - 排查：检查 should_stop_callback 函数逻辑和中断检查间隔
  - 参考路径：[中断移动实现:369-437](file://src/combat/movement_controller.py#L369-L437)
- **优化**：日志相关问题
  - 现象：日志输出过多或过少
  - 排查：检查日志级别配置，确认已删除冗余日志但仍保留关键信息
  - 参考路径：[日志精简实现:273-275](file://src/combat/movement_controller.py#L273-L275)
...
- [movement_controller.py:461-511](file://src/combat/movement_controller.py#L461-L511)
- [movement_controller.py:729-800](file://src/combat/movement_controller.py#L729-L800)
- [movement_controller.py:369-437](file://src/combat/movement_controller.py#L369-L437)

## 结论
ok-jump 的移动控制系统通过"状态感知 + 任务编排 + 距离驱动"的架构，实现了在复杂战场态势下的智能移动与技能释放。系统在后台模式与 ADB 模式下均具备稳定表现，配合滞后效应与防抖动机制，显著提升了移动与战斗的可靠性。

**更新** 本次更新引入的移动速度跟踪机制和可中断移动功能，使系统能够根据实际移动表现动态调整移动策略，大幅提升了移动控制的精度和响应速度。通过合理的参数配置与调优，可在不同分辨率与设备环境下获得一致的体验。同时，日志优化删除了约46行冗余信息，提升了系统运行效率，为后续功能扩展奠定了更好的基础。
...
- 距离与移动参数
  ...
  - 边界缓冲区：15px，避免边界抖动
  - 移动阈值：30px，过滤微小偏移
  - **新增**：速度采样窗口：默认10次，可根据设备性能调整
  - **新增**：最大移动时间：默认2秒，防止过度移动
- **优化**：日志配置
  - 精简日志输出：删除冗余调试信息，保留关键错误和状态日志
  - 性能提升：减少 I/O 开销，提升系统响应速度
- 示例路径（不展示代码，仅提供定位）
  ...
  - **新增**：[速度采样与动态时间计算:729-800](file://src/combat/movement_controller.py#L729-L800)
  - **优化**：[日志精简实现:273-275](file://src/combat/movement_controller.py#L273-L275)
