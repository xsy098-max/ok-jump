...
- **新增**：YOLO自检战斗状态检测能力，通过自身检测判断战斗场景的动态启停
...
- StateDetector：基于YOLO的战场状态检测器，支持并行死亡监控、多标签检测和**YOLO自检战斗状态检测**
...
## 架构总览
自动战斗系统采用"配置驱动 + 并行监控 + 状态机"的设计，**新增**了YOLO自检战斗状态检测能力：
- 配置驱动：技能开关与间隔来自AutoCombatTask.json；按键映射来自游戏热键配置.json
- 并行监控：死亡状态在独立线程高频检测，主线程快速查询
- **YOLO自检战斗状态检测**：通过YOLO自身检测判断战斗场景的动态启停，避免固定时间间隔的战斗循环
...
Task->>SD : 启动YOLO自检战斗状态检测
loop 状态感知主循环
SD->>SD : check_combat_state_by_self_detection()
SD->>YOLO : detect_self_once()
YOLO-->>SD : DetectionResult/None
SD-->>Task : (in_combat, state_changed)
alt 检测到进入战斗
...
- [state_detector.py:508-588](file://src/combat/state_detector.py#L508-L588)
...
### YOLO自检战斗状态检测能力
**新增功能**：通过YOLO自身检测判断战斗场景的动态启停，这是战斗系统的核心改进

- **防抖动机制**：连续2次检测到自身才确认进入战斗，连续3次未检测到自身才确认退出战斗
- **状态切换**：自动战斗线程的启动和停止完全由YOLO检测结果驱动
- **性能优化**：避免了固定时间间隔的战斗循环，只有在检测到战斗场景时才启动战斗线程

```mermaid
flowchart TD
Start([状态感知主循环]) --> Detect["check_combat_state_by_self_detection()"]
Detect --> Found{"检测到自身?"}
Found --> |是| ConsecutiveFound["连续检测到自身计数+1"]
ConsecutiveFound --> Threshold1{"连续>=2次?"}
Threshold1 --> |是| EnterCombat["进入战斗状态<br/>启动战斗线程"]
Threshold1 --> |否| Wait1["等待"]
Found --> |否| ConsecutiveNot["连续未检测到自身计数+1"]
ConsecutiveNot --> Threshold2{"连续>=3次?"}
Threshold2 --> |是| ExitCombat["退出战斗状态<br/>停止战斗线程"]
Threshold2 --> |否| Wait2["等待"]
EnterCombat --> Wait3["等待下次检测"]
ExitCombat --> Wait4["等待下次检测"]
Wait1 --> Detect
Wait2 --> Detect
Wait3 --> Detect
Wait4 --> Detect
...
**图表来源**
- [state_detector.py:508-588](file://src/combat/state_detector.py#L508-L588)
- [AutoCombatTask.py:360-416](file://src/task/AutoCombatTask.py#L360-L416)
...
- **YOLO自检战斗状态检测**：通过防抖动机制避免状态频繁切换，减少不必要的线程启动/停止开销
...
- **YOLO自检战斗状态检测异常**：检查自身检测的连续阈值配置，确认防抖动机制正常工作
...
## 结论
OK-Jump战斗系统通过"配置驱动 + 并行监控 + 状态机"的架构实现了稳定高效的自动战斗体验。**新增的YOLO自检战斗状态检测能力**使得系统能够智能地动态启停战斗线程，避免了固定时间间隔的资源浪费。YOLO模型提供可靠的多标签检测，滞回距离控制与后台适配确保了流畅与鲁棒性。建议在实际使用中：
- 调整AutoCombatTask.json中的技能间隔与移动持续时间
- 根据游戏窗口状态启用后台模式与伪最小化
- 结合实际场景微调YOLO置信度与检测标签
- **合理配置YOLO自检战斗状态检测的防抖动阈值**
