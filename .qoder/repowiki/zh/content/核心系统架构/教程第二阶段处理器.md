...
## 更新摘要
**所做更改**
- 新增手动配置加载机制章节，详细介绍如何从配置文件加载 AutoCombatTask 参数
- 更新并行处理战斗线程和结束检测机制，包括独立线程管理和同步控制
- 增强自动战斗系统继承用户配置功能，确保新手教程使用GUI配置参数
- 更新智能战斗检测功能，包括模板匹配 fight_start.png 和 OCR 文本检测
- 增强双加载界面等待机制，增加战斗开始标志检测功能
- 增强容错处理机制，支持积分争夺场景的智能检测
- 更新配置参数说明，添加相关的新配置项
- 增强超时配置和状态管理机制
...
**更新** 新增手动配置加载机制，确保新手教程中的自动战斗使用用户在GUI中配置的参数。同时增强了并行处理战斗线程和结束检测的功能，实现了独立的战斗线程和结束检测线程，提高了系统的响应性和稳定性。此外，更新了智能战斗检测功能，包括模板匹配 fight_start.png 和 OCR 文本检测（积分争夺/積分爭奪），以及增强的双加载界面等待机制，支持战斗开始标志检测。
...
**更新** 新增手动配置加载机制，通过 `_get_combat_config` 方法实现从配置文件加载 AutoCombatTask 参数的功能。同时增强了并行处理战斗线程和结束检测的功能，实现了独立的战斗线程和结束检测线程，提高了系统的响应性和稳定性。
...
- **手动配置加载机制**：确保新手教程中的自动战斗使用用户在GUI中配置的参数
...
**更新** 新增的手动配置加载机制显著提升了系统的灵活性和用户体验，确保新手教程能够正确继承用户在GUI中配置的各项参数。同时增强的并行处理战斗线程和结束检测功能，以及智能战斗检测功能，进一步提升了系统的鲁棒性和适应性。
