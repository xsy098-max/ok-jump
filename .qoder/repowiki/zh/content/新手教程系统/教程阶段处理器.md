...
图表来源
- [phase1_handler.py:1-1137](file://src/tutorial/phase1_handler.py#L1-L1137)
- [phase2_handler.py:1-1478](file://src/tutorial/phase2_handler.py#L1-L1478)
...
- 关键特性
  ...
  - **新增功能**：增强的抖动检测与敌人位置平滑机制，解决 YOLO 检测抖动问题。
- 数据流
  - 状态机驱动：状态切换决定执行分支。
...
- 关键特性
  ...
  - **新增功能**：改进的 GUI 自动战斗触发器隔离，避免影响用户手动控制。
- 数据流
  - 检测器负责模板匹配与 OCR。
...
- **新增功能**：增强的 OCR 文字匹配支持简繁双语，提升国际化支持。
...
### 抖动检测与敌人位置平滑机制（Phase1Handler 增强）
- **新增功能**：增强的抖动检测系统
  - 坐标历史记录：记录最近4个位置，检测 A→B→A→B 模式
  - 移动方向历史：记录最近6个移动方向，检测左右左右左右模式
  - 方向抖动检测：识别 [-1, +1, -1, +1, -1, +1] 模式
- **新增功能**：敌人位置平滑机制
  - 历史位置记录：最多记录5个敌人位置
  - 中位数计算：使用中位数对异常值更鲁棒
  - 位置跳动检测：超过150像素的跳动被视为异常
  - 加权平均：对历史位置进行加权平均，越近的权重越高
- **应用场景**：解决 YOLO 检测猴子时的抖动问题，提升移动控制精度

章节来源
- [phase1_handler.py:46-62](file://src/tutorial/phase1_handler.py#L46-L62)
- [phase1_handler.py:972-985](file://src/tutorial/phase1_handler.py#L972-L985)
- [phase1_handler.py:987-1056](file://src/tutorial/phase1_handler.py#L987-L1056)

### GUI 自动战斗触发器隔离（Phase2Handler 改进）
- **关键修复**：只停止第二阶段自己创建的战斗实例
- **设计原则**：GUI 触发器由用户控制启停，第二阶段不应干预
- **实现方式**：区分手动创建的战斗实例与 GUI 自动战斗触发器
- **效果**：避免影响用户的手动战斗控制

章节来源
- [phase2_handler.py:506-508](file://src/tutorial/phase2_handler.py#L506-L508)
...
- OCR 与模板匹配
  ...
  - OCR 结果缓存与清理，避免重复计算。
  - **新增**：简繁双语支持，提升国际化检测准确性。
- 卡顿与抖动检测
  - 位置历史与方向历史记录，配合抖动检测与随机移动，提升移动稳定性。
  - **新增**：中位数计算对异常值更鲁棒，加权平均提升平滑效果。
- 资源管理
  - 线程启动/停止与资源清理，防止内存泄漏与线程悬挂。
  - **新增**：GUI 自动战斗触发器隔离，避免资源冲突。
...
- 常见问题与定位
  ...
  - 主界面验证失败：增加重试次数与等待时间。
  - **新增**：猴子检测抖动：检查 YOLO 模型，启用位置平滑机制。
  - **新增**：GUI 控制冲突：确认第二阶段未干预用户手动战斗。
- 日志与截图
...
## 结论
教程阶段处理器通过状态机与检测器的协同，实现了从角色选择到自动战斗的完整自动化流程。第一阶段侧重教学引导与基础战斗准备，第二阶段侧重实战与流程收尾。两阶段之间通过并行结束检测与自动战斗实现无缝衔接，具备完善的超时、重试与容错机制。**最新增强**包括：改进的抖动检测与敌人位置平滑机制、GUI 自动战斗触发器隔离、简繁双语支持等，进一步提升了系统的稳定性与用户体验。开发者可基于现有框架扩展新角色、优化检测策略或调整流程细节。
