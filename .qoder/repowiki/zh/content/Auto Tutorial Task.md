...
## 更新摘要
**所做更改**
- 更新角色选择配置章节，反映从'悟空'到'小鸣人'的默认配置变更
- 更新配置文件说明，强调新的默认角色设置
- 更新角色选择器配置，说明小鸣人的特性和配置差异
- 更新测试用例相关说明，反映执行顺序的变化
...
**更新** 系统现已默认配置为使用'小鸣人'角色进行新手教程执行，同时保持对其他角色的完整支持。系统支持多种角色选择模式，包括单角色执行和全角色批量执行，并集成了先进的YOLO目标检测、OCR文字识别和模板匹配技术。系统采用状态机管理模式，确保复杂的教程流程能够稳定可靠地执行。
...
**更新** 系统现已默认使用小鸣人角色进行新手教程执行，但保持对所有角色的完整支持。小鸣人角色具有独特的右侧区域点击特性和目标圈检测能力。
...
**更新** 系统现已默认配置为使用'小鸣人'角色，这反映了该角色在新手教程流程中的特殊优势。用户仍可根据需要选择其他角色或使用'全部'模式进行完整测试。
