...
图表来源
- [AutoMatchTask.py:21-104](file://src/task/AutoMatchTask.py#L21-L104)
- [BaseJumpTask.py:14-422](file://src/task/BaseJumpTask.py#L14-L422)
- [mixins.py:15-774](file://src/task/mixins.py#L15-L774)
...
- 无法进入大厅
  ...
  - 参考
    - [AutoMatchTask.py:56-67](file://src/task/AutoMatchTask.py#L56-L67)
    - [features.py:64-68](file://src/constants/features.py#L64-L68)
- 开始匹配按钮未识别
  - 现象：特征匹配不到“开始匹配”按钮
  - 排查：确认coco_detection.json中存在对应类别；检查模板质量与阈值；必要时使用相对坐标点击
  - 参考
    - [AutoMatchTask.py:69-81](file://src/task/AutoMatchTask.py#L69-L81)
...
## 结论
AutoMatchTask通过特征匹配与相对坐标点击实现稳定的匹配流程，结合BaseJumpTask与JumpTaskMixin提供的分辨率适配、后台输入与伪最小化能力，能够在复杂环境下可靠运行。合理配置AutoMatchTask.json与devices.json，并遵循性能与故障排除建议，可显著提升匹配成功率与稳定性。
...
- 运行监控
  ...
  - 参考
    - [AutoMatchTask.py:22-53](file://src/task/AutoMatchTask.py#L22-L53)
...
- 异常处理
  ...
  - 参考
    - [AutoMatchTask.py:40-51](file://src/task/AutoMatchTask.py#L40-L51)
- 性能优化
  - 动态调整最大等待时间
  - 优先使用特征匹配而非OCR
