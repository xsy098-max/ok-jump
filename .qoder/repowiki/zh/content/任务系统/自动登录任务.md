...
```mermaid
flowchart TD
Start([配置参数]) --> Enable["启用<br/>默认: true"]
Enable --> AutoStart["自动启动游戏<br/>默认: false"]
AutoStart --> WaitTime["等待游戏启动(秒)<br/>默认: 120"]
WaitTime --> MaxAttempts["最大登录尝试次数<br/>默认: 10"]
MaxAttempts --> InputAccount["输入账号<br/>默认: true"]
InputAccount --> Account["账号<br/>默认: 空字符串"]
Account --> RetryCount["账号输入重试次数<br/>默认: 5"]
RetryCount --> VerifyTimeout["输入校验超时(秒)<br/>默认: 30.0"]
