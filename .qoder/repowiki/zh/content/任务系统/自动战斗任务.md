...
## 更新摘要
**所做更改**
- 更新了技能范围从250px到225px的变更说明，反映最新的距离控制逻辑
- 修正了距离控制范围的描述，确保与代码实现一致
- 更新了相关配置参数和日志输出的准确性
- 完善了技能释放范围的说明，确保战斗逻辑的一致性
...
- DistanceCalculator：带滞后缓冲的距离控制，维持0~225像素的最佳攻击距离
...
- **智能释放**：根据距离范围（0-225px）自动判断是否释放技能
...
- 距离计算：欧氏距离
- 距离判定：带滞后缓冲的范围判断（进入/离开使用不同阈值），维持0~225像素
- 方向建议：靠近/远离/停止
...
Calc --> Range{"距离范围?"}
Range --> |<0px| Away["远离目标"]
Range --> |0-225px| Stop["停止移动"]
Range --> |>225px| Toward["靠近目标"]
Away --> End([返回])
Stop --> End
Toward --> End
```
...
### 实际游戏场景示例
- 仅敌方场景：锁定最近敌人，维持0~225像素距离，达标后启动自动技能，移动中停止技能
...
### 技能范围变更说明
**重要更新**：技能范围已从250px调整为225px，这一变更影响了所有距离控制逻辑：

- **距离控制范围**：从原来的0~250px调整为0~225px
- **技能释放范围**：0-225px范围内的目标将触发技能释放
- **移动策略**：靠近目标的阈值从250px调整为225px
- **滞后缓冲**：缓冲区设置为15px，避免边界值附近频繁切换状态

这些变更确保了战斗逻辑的一致性和稳定性，提高了技能释放的准确性和响应速度。

**章节来源**
- [distance_calculator.py:30-31](file://src/combat/distance_calculator.py#L30-L31)
- [skill_controller.py:146-147](file://src/combat/skill_controller.py#L146-L147)
- [AutoCombatTask.py:433](file://src/task/AutoCombatTask.py#L433)
- [AutoCombatTask.py:510](file://src/task/AutoCombatTask.py#L510)
- [AutoCombatTask.py:718](file://src/task/AutoCombatTask.py#L718)