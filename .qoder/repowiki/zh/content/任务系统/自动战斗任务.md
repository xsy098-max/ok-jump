...
## 更新摘要
**变更内容**
- 完全重构战斗循环系统为扁平循环架构，消除嵌套 while 循环
- 新增 detect_all_once() 方法，实现单次全量检测优化
- 增强抗卡死和抖动检测机制，包含卡住检测和抖动检测
- 优化技能生命周期管理，仅在状态切换时启动/停止技能
- 改进目标锁定机制，支持目标丢失检测和重新锁定
- 优化性能，减少 YOLO 推理次数，提升检测效率
- **日志优化**：大幅减少战斗循环日志、状态检测日志和技能配置dump日志，保留关键战斗信息而非详细帧信息
...
**图表来源**
- [AutoCombatTask.py:1-1514](file://src/task/AutoCombatTask.py#L1-L1514)
- [state_detector.py:1-632](file://src/combat/state_detector.py#L1-L632)
- [skill_controller.py:1-589](file://src/combat/skill_controller.py#L1-L589)
- [movement_controller.py:1-760](file://src/combat/movement_controller.py#L1-L760)
...
- StateDetector：基于 YOLO 的战场状态检测器，支持死亡状态并行监控、自身检测、友方/敌方检测、战斗状态判断，新增 detect_all_once() 单次全量检测方法。
- SkillController：技能释放控制器，支持后台模式、独立冷却、多技能并发控制，优化技能生命周期管理。
...
## 架构总览
自动战斗任务采用"任务编排 + 模块化控制器"的架构设计，经过重构后实现扁平循环架构：
- 任务层：AutoCombatTask 作为触发任务，负责生命周期管理、状态感知主循环、战斗线程调度与资源清理。
- 检测层：StateDetector 提供 YOLO 检测能力，支持死亡状态并行监控与战斗状态判断，新增 detect_all_once() 单次全量检测。
...
Task->>SD : detect_all_once()
SD-->>Task : self_pos, allies, enemies, has_death
Task->>DC : 计算与目标距离
DC-->>Task : 距离值
...
### AutoCombatTask 类分析
AutoCombatTask 是自动战斗的核心编排器，经过重构后实现扁平循环架构，负责：
...
- 扁平战斗执行循环：在独立线程中实现单循环设计，每轮1次截帧+1次 YOLO 推理，消除嵌套循环。
- 死亡状态监控：合并到主循环的防抖动死亡检测，提升响应速度。
- 卡住/抖动检测：新增卡住检测和抖动检测机制，通过位置历史分析摆脱困境。
- 目标锁定机制：实现智能目标锁定，支持目标丢失检测和重新锁定。
- **日志优化**：大幅减少战斗循环中的详细日志输出，仅保留关键状态信息，避免日志刷屏。
...
- 单次全量检测：detect_all_once() 实现1帧+1次 YOLO推理，相比传统3次推理开销降低约66%。
- **日志优化**：死亡监控线程的详细日志已被简化，仅保留关键状态变化信息。
...
- 技能生命周期优化：仅在状态切换时启动/停止技能，减少不必要的资源消耗。
- **日志优化**：技能线程启动、监控循环和技能释放的日志已被禁用，避免日志过多。
...
- 目标锁定：_find_locked_target() 实现智能目标锁定，支持目标丢失检测和重新锁定。
- **日志优化**：移动控制中的详细日志（如按键异常、偏移过小等）已被简化，仅保留关键信息。
...
### 抗卡死和抖动检测机制
- 卡住检测：_detect_stuck() 检测角色是否被卡住（连续4次检测到相同坐标），阈值10像素内视为同一位置。
- 抖动检测：_detect_jitter() 检测A-B-A-B抖动模式，将历史位置分为偶数组和奇数组分别聚类分析。
- 随机移动：_perform_random_move() 执行随机移动摆脱卡住或抖动状态。
- 位置历史管理：_record_position() 记录位置历史，支持最大长度限制。

```mermaid
flowchart TD
Start(["位置检测"]) --> RecordPos["记录当前位置"]
RecordPos --> CheckStuck{"卡住检测？"}
CheckStuck --> |是| StuckAction["向下移动1秒"]
StuckAction --> ClearHistory["清空历史"]
ClearHistory --> End["结束"]
CheckStuck --> |否| CheckJitter{"抖动检测？"}
CheckJitter --> |是| RandomMove["随机移动2秒"]
RandomMove --> ClearHistory
CheckJitter --> |否| Normal["正常移动"]
Normal --> End
```

**图表来源**
- [AutoCombatTask.py:1408-1443](file://src/task/AutoCombatTask.py#L1408-L1443)
- [AutoCombatTask.py:1444-1500](file://src/task/AutoCombatTask.py#L1444-L1500)
- [AutoCombatTask.py:1502-1518](file://src/task/AutoCombatTask.py#L1502-L1518)
...
- StateDetector 依赖 CombatLabel 与 YOLO 检测结果，提供 BattlefieldState 枚举，新增 detect_all_once() 单次全量检测方法。
...
- StateDetector 依赖 CombatLabel 与 YOLO 检测，新增 detect_all_once() 方法。
...
## 性能考量
- 扁平循环架构：消除嵌套 while 循环，每轮仅1次截帧+1次 YOLO 推理，显著提升性能。
- 单次全量检测：detect_all_once() 方法将原本3次 YOLO 推理合并为1次，开销降低约66%。
- 技能生命周期优化：仅在状态切换时启动/停止技能，减少不必要的资源消耗。
- 目标锁定机制：智能目标锁定减少重复检测，提升目标跟踪效率。
- 抗卡死和抖动检测：通过位置历史分析及时摆脱困境，避免无效循环。
- 死亡防抖：合并到主循环的防抖动机制，提升响应速度和准确性。
- **日志优化**：大幅减少日志输出，避免日志 I/O 成为性能瓶颈。
...
- 卡住检测误报：调整卡住检测阈值和计数，避免正常移动被误判为卡住。
- 抖动检测误报：检查抖动检测配置，确保阈值适合当前游戏场景。
- **日志问题**：由于日志优化，某些详细信息可能不再显示。如需调试，可临时启用详细日志模式。
...
## 结论
AutoCombatTask 通过模块化设计实现了稳定的自动战斗逻辑：状态检测、技能控制、移动控制与距离计算协同工作，结合后台输入支持与配置驱动，能够在不同环境下可靠运行。经过重构后，扁平循环架构、单次全量检测、抗卡死和抖动检测机制以及优化的技能生命周期管理显著提升了性能和稳定性。**日志优化**进一步减少了系统开销，使战斗过程更加流畅。通过合理配置与参数调优，可进一步提升战斗效率与稳定性。
...
- 配置选项说明（来自 AutoCombatTask.json 与 GUI 配置）
  ...
  - 卡住检测：启用后检测角色是否卡住（连续多次坐标不变），自动尝试摆脱。
  - 抖动检测：启用后检测A-B-A-B位置抖动模式，自动执行随机移动摆脱。
- YOLO 标签定义（CombatLabel）
  ...
  - TARGET_CIRCLE：目标圈
- **日志优化说明**
  - 技能释放日志：技能线程启动、监控循环、技能释放等日志已被禁用
  - 移动控制日志：按键异常、偏移过小等详细日志已被简化
  - 状态检测日志：死亡监控线程的详细日志已被简化
  - 战斗循环日志：战斗过程中的详细帧信息日志已被大幅减少
