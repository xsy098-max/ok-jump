...
## 项目结构
OK-Jump 的触发器任务位于 src/task 目录，采用“基类 + Mixin”的设计模式，通过 Mixin 复用通用能力（分辨率适配、后台模式支持、输入适配等）。AutoCombatTask 是典型的触发器任务示例，基于 BaseJumpTriggerTask 实现自动战斗逻辑。
...
## 架构总览
触发器任务的运行时架构如下：任务通过 Mixin 获取通用能力，结合战斗子系统进行状态检测与动作执行；后台模式下通过 SendInput 实现可靠输入；分辨率适配保证不同屏幕下的坐标一致性。

```mermaid
graph TB
A["触发器任务<br/>BaseJumpTriggerTask"] --> B["混入能力<br/>JumpTaskMixin"]
B --> C["后台管理<br/>BackgroundManager"]
B --> D["后台输入<br/>BackgroundInputHelper"]
B --> E["分辨率适配<br/>ResolutionAdapter"]
A --> F["状态检测<br/>StateDetector"]
A --> G["移动控制<br/>MovementController"]
A --> H["技能控制<br/>SkillController"]
F --> I["YOLO检测<br/>CombatLabel"]
G --> D
H --> D
```
...
## 依赖分析
- 组件耦合：AutoCombatTask 依赖 JumpTaskMixin 提供的通用能力；战斗子系统彼此解耦，通过任务对象共享帧与日志。
...
A --> E["SkillController"]
B --> F["BackgroundManager"]
B --> G["BackgroundInputHelper"]
B --> H["ResolutionAdapter"]
```
...
## 结论
BaseJumpTriggerTask 通过 Mixin 模式将通用能力抽象复用，配合 AutoCombatTask 的触发机制与战斗子系统，实现了稳定高效的触发器任务体系。通过后台管理、输入适配与分辨率适配，任务可在复杂环境下保持可靠运行；通过配置驱动与详细日志，开发者能够灵活定制并高效调试。
