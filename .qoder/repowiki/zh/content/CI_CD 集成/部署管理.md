...
**更新** 本版本反映了部署管理流程的简化，移除了 PyInstaller 相关配置，采用新的 pyappify 打包方案。

## 项目结构
ok-jump 的 CI/CD 与部署相关代码集中在 src/ci 目录，配合测试与配置文件共同构成完整的自动化测试与部署体系。项目现已采用 pyappify 作为主要的打包工具，替代了传统的 PyInstaller 方案。
...
subgraph "打包配置"
PA["pyappify.yml<br/>pyappify配置"]
MP["main.py<br/>主程序入口"]
MD["main_debug.py<br/>调试模式"]
end
subgraph "外部依赖"
...
- [main.py:1-761](file://main.py#L1-L761)
...
- 打包配置（pyappify）：新的打包方案配置，支持多配置文件、Python 版本指定、依赖管理等。

**更新** 新增了 pyappify 打包配置组件，替代了原有的 PyInstaller 方案。
...
### 打包配置（pyappify）
- 职责
  - 定义应用程序的打包配置，支持多配置文件、Python 版本指定、依赖管理等。
  - 配置中国版和调试版两个打包配置文件。
  - 指定主程序入口、依赖文件、Python 版本要求等。
- 关键配置
  - 中国版配置：使用腾讯云镜像源加速、启用管理员权限、使用 pythonw 隐藏控制台。
  - 调试版配置：禁用 pythonw，便于查看控制台输出。
  - 通用配置：指定主程序文件、依赖文件、pip 参数等。
- 复杂度
  - 配置解析：O(1)，配置项数量固定。
  - 打包执行：受依赖数量和文件大小影响。
...
```mermaid
flowchart TD
Start(["开始打包"]) --> LoadConfig["加载pyappify.yml"]
LoadConfig --> ParseProfiles["解析配置文件"]
ParseProfiles --> CheckMainScript["检查主程序入口"]
CheckMainScript --> CheckRequirements["检查依赖文件"]
CheckRequirements --> SetupPython["设置Python环境"]
SetupPython --> InstallDeps["安装依赖"]
InstallDeps --> BuildExe["构建可执行文件"]
BuildExe --> PackageDist["打包分发文件"]
PackageDist --> End(["打包完成"])
```
...
- 打包依赖
  - pyappify 作为新的打包工具，替代了 PyInstaller，提供更简洁的配置方式和更好的 Windows 集成。

**更新** 新增了 pyappify 打包工具的依赖关系分析。
...
- 打包性能
  - pyappify 相比 PyInstaller 更加轻量，启动速度更快，内存占用更低。
  - 多配置文件支持减少了重复打包的工作量。

**更新** 新增了 pyappify 打包工具的性能考量。
...
- 打包配置问题
  - 现象：pyappify 打包失败或配置不生效。
  - 排查：检查 pyappify.yml 配置语法；确认主程序入口和依赖文件路径；验证 Python 版本要求。
  - 参考
    - [pyappify.yml:1-18](file://pyappify.yml#L1-L18)
    - [main.py:1-761](file://main.py#L1-L761)
    - [main_debug.py:1-16](file://main_debug.py#L1-L16)

**更新** 新增了 pyappify 打包配置相关的故障排除指南。
...
## 结论
ok-jump 的部署管理系统围绕"包管理—模拟器—部署—异常处理—结果管理"形成闭环，具备良好的可扩展性与可维护性。通过合理的超时控制、异常恢复与报告机制，能够支撑稳定的 CI/CD 流程。项目现已采用 pyappify 作为新的打包工具，相比传统方案更加轻量高效。建议在生产环境中进一步完善下载校验、镜像加速与清理策略，并结合蓝绿/灰度/回滚策略提升发布质量与风险控制能力。

**更新** 结论部分反映了新的 pyappify 打包方案的优势。
...
- 打包策略
  - 使用 pyappify 的多配置文件功能，分别维护中国版和调试版的打包配置。
  - 中国版使用镜像源加速，调试版便于开发调试。
  - 定期更新依赖版本，确保打包的一致性和安全性。

**更新** 新增了基于 pyappify 的打包策略建议。

### 运维实用清单
- 环境准备
  - 安装并配置雷电模拟器，确保 ldconsole 可用；设置 ADB 端口与实例索引。
  - 安装 Python 依赖：参考 requirements.txt。
  - 配置 pyappify 打包环境，确保主程序入口和依赖文件正确。
- 日志与截图
  - 异常发生时自动保存截图与堆栈；测试报告按日期/时间归档。
- 清理策略
  - 定期清理旧 APK 与测试结果，避免磁盘压力过大。
  - pyappify 打包产物按配置自动清理，避免重复打包占用空间。
- CI 集成
  - GitHub Actions 构建与发布流程可直接复用，按需调整版本号与发布内容。
  - pyappify Action 提供了简化的打包流程，减少手动配置工作量。

**更新** 新增了 pyappify 打包相关的运维建议。
