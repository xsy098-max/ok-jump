...
**重要更新**：MainWindowTask现在默认从GUI界面中隐藏，但仍可通过命令行模式或直接调用的方式使用。这一变更不影响功能的可用性，仅改变了用户的访问方式。
...
**重要变更**：MainWindowTask现在通过OK框架的任务管理系统注册，但默认不显示在GUI界面的任务列表中，需要通过命令行参数或直接调用的方式访问。
...
**重要更新**：MainWindowTask现在默认从GUI界面中隐藏，这一变更提升了GUI界面的简洁性，同时通过命令行模式和直接调用方式保持了完整的功能可用性。开发者可以根据需求选择合适的访问方式。
