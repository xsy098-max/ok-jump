...
## 更新摘要
**所做更改**
- 更新了全局窗口配置部分，反映窗口位置坐标从(1020, 320)调整为(828, 564)，尺寸从(1296, 748)调整为(1031, 655)的变更
- 新增了窗口几何配置的详细说明，包括window_x、window_y、window_width、window_height等参数
- 补充了窗口位置坐标的历史变更记录和当前状态
...
**更新** 本次更新重点关注全局窗口配置的调整，包括窗口位置坐标和尺寸的优化，实现了更小、更居中的窗口布局。
...
**章节来源**
- [config.py:1-148](file://config.py#L1-L148)
...
- 窗口几何配置
  - window_x：整数，窗口左上角X坐标，默认241。
  - window_y：整数，窗口左上角Y坐标，默认575。
  - window_width：整数，窗口宽度，默认1031。
  - window_height：整数，窗口高度，默认655。
...
- 一次性任务与触发任务
  - onetime_tasks：列表，一次性任务的模块与类名，包含主窗口、自动登录、教程、匹配、日常等任务。
...
## 架构总览
下图展示了配置系统在运行时的交互关系：主配置文件提供默认配置；ConfigOption封装配置组；运行时模块从全局配置中读取参数；JSON配置文件作为外部持久化配置与主配置协同工作。
...
- 当前版本：window_x=828, window_y=564, window_width=1031, window_height=655
...
### 一次性任务与触发任务
- onetime_tasks：一次性任务列表，包含主窗口、自动登录、教程、匹配、日常等任务。
...
**章节来源**
- [config.py:143-147](file://config.py#L143-L147)
...
- 窗口尺寸优化：新的窗口尺寸(1031x655)相比之前的(1296x748)减少了约20%的像素占用，有助于提升整体性能。
...
**更新** 最新更新反映了全局窗口配置的优化调整，新的窗口位置(828, 564)和尺寸(1031x655)实现了更居中、更紧凑的布局，提升了用户体验。建议在部署前根据实际需求调整分辨率、窗口与ADB配置，并利用日志与截图路径进行问题定位。
