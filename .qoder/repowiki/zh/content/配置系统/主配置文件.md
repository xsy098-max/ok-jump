...
- JSON配置文件：Basic Options.json、游戏热键配置.json、devices.json、ui_config.json等作为外部持久化配置，与主配置协同工作。
...
## 结论
主配置文件config.py为OK-Jump提供了完整的配置骨架，结合ConfigOption类与运行时模块，实现了灵活的配置管理与强大的扩展能力。通过合理设置各配置项，可在不同平台与环境下获得稳定的自动化体验。建议在部署前根据实际需求调整分辨率、窗口与ADB配置，并利用日志与截图路径进行问题定位。
