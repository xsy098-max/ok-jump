...
- 新增UI改进补丁，修复任务按钮对齐问题
- 更新窗口几何配置，反映新的窗口位置坐标(984, 486)和尺寸(1031x682)
...
**更新** 本次更新重点关注配置系统的稳定性改进，包括ADB连接错误处理优化、UI按钮对齐修复以及窗口几何配置的更新，提升了整体用户体验和系统可靠性。
...
- 窗口几何配置
  - window_x：整数，窗口左上角X坐标，默认984。
  - window_y：整数，窗口左上角Y坐标，默认486。
  - window_width：整数，窗口宽度，默认1031。
  - window_height：整数，窗口高度，默认682。
...
- 当前版本：window_x=984, window_y=486, window_width=1031, window_height=682
...
- 窗口尺寸优化：新的窗口尺寸(1031x682)相比之前的(1296x748)减少了约20%的像素占用，有助于提升整体性能。
...
**更新** 最新更新反映了配置系统的多项重要改进：ADB连接错误处理补丁显著减少了日志冗余，任务按钮对齐补丁提升了UI一致性，窗口几何配置的优化实现了更好的视觉效果。建议在部署前根据实际需求调整分辨率、窗口与ADB配置，并利用日志与截图路径进行问题定位。运行时补丁系统进一步增强了系统的稳定性和用户体验。
