...
class TutorialDetector:
    ...
    def detect_character_select_screen(self, timeout: float = 10.0) -> bool:
        ...
        检测方法（按优先级）：
        1. 模板匹配 Features.XUANREN
        2. OCR 检测 "请选择一位你心仪的角色" 文字
        ...
        while time.time() - start_time < timeout:
            ...
            # 方法1: 模板匹配（优先，与 AutoLoginTask 保持一致）
            try:
                xuanren = self.task.find_one(Features.XUANREN, threshold=0.6)
                if xuanren:
                    self._log("模板匹配检测到选角界面")
                    return True
            except ValueError:
                pass
            except Exception as e:
                self._log(f"模板匹配异常: {e}")
